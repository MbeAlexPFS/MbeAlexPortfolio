                ...
                <div class="flex flex-wrap items-center gap-4">
                    ...
                    @auth
                        @if(Auth::user()->isAdmin())
                            <a href="{{ route('admin.dashboard') }}" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition">Administration</a>
                        @endif
                        <form method="POST" action="{{ route('auth.logout') }}" class="inline">
                            @csrf
                            <button type="submit" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition">Déconnexion</button>
                        </form>
                    @else
                        <a href="{{ route('auth.login') }}" class="hover:text-indigo-600 dark:hover:text-indigo-400 transition">Admin</a>
                    @endauth
                </div>
            </div>
        </div>
