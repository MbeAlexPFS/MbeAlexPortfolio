...
@section('content')
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <a href="{{ route('admin.blog.index') }}" class="text-sm text-gray-500 hover:text-indigo-600 transition">&larr; Retour</a>
        <h1 class="mt-4 text-3xl font-bold text-gray-900">{{ isset($article) ? 'Modifier l\'article' : 'Nouvel article' }}</h1>
        ...
        <form method="POST" action="{{ isset($article) ? route('admin.blog.update', $article) : route('admin.blog.store') }}" class="mt-8 space-y-5">
            ...
            <div>
                ...
                <textarea name="excerpt" id="excerpt" rows="2"
                    class="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">{{ old('excerpt', $article->excerpt ?? '') }}</textarea>
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="15" required
                    class="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">{{ old('content', $article->content ?? '') }}</textarea>
            ...
            <div class="flex items-center gap-3">
                ...
                <label for="is_published" class="text-sm text-gray-700">Publier</label>
            </div>
