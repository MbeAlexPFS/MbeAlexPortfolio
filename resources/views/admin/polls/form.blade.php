...
@section('content')
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <a href="{{ route('admin.polls.index') }}" class="text-sm text-gray-500 hover:text-indigo-600 transition">&larr; Retour</a>
        <h1 class="mt-4 text-3xl font-bold text-gray-900">{{ isset($poll) ? 'Gérer le sondage' : 'Nouveau sondage' }}</h1>
        ...
        <form method="POST" action="{{ isset($poll) ? route('admin.polls.update', $poll) : route('admin.polls.store') }}" class="mt-8 space-y-5">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                    class="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">{{ old('description', $poll->description ?? '') }}</textarea>
            </div>
        ...
        @if(isset($poll))
                ...
                @foreach($poll->questions as $question)
                    <div class="bg-white border border-gray-200 rounded-xl p-5">
                        <div class="flex items-center justify-between">
                            <div>
                                <span class="text-xs text-indigo-600 uppercase tracking-wider">{{ str_replace('_', ' ', $question->type) }}</span>
                                <h3 class="font-medium text-gray-900 mt-0.5">{{ $question->text }}</h3>
                                @if($question->options->isNotEmpty())
                                    <div class="mt-2 flex flex-wrap gap-1.5">
                                        @foreach($question->options as $opt)
                                            <span class="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded">{{ $opt->text }}</span>
                                        @endforeach
                                    </div>
                                @endif
                            </div>
            ...
            <div class="mt-8 bg-white border border-gray-200 rounded-xl p-6">
                ...
                <form method="POST" action="{{ route('admin.polls.questions.store', $poll) }}" class="mt-4 space-y-4">
                    @csrf
                    <div>
                        <label for="text" class="block text-sm font-medium text-gray-700">Texte de la question</label>
                    ...
                    <div id="scale-fields" class="hidden grid sm:grid-cols-2 gap-4">
                        <div>
                            <label for="scale_min" class="block text-sm font-medium text-gray-700">Note minimale</label>
                            <input type="number" name="scale_min" id="scale_min" value="1" min="1"
                                class="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                        </div>
                        <div>
                            <label for="scale_max" class="block text-sm font-medium text-gray-700">Note maximale</label>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="is_required" value="1" id="is_required" checked
                            class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                        <label for="is_required" class="text-sm text-gray-700">Obligatoire</label>
                    </div>
